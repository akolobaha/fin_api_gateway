...
func FindOrCreateToken(userId int64, conn *gorm.DB) (*UserToken, error) {
	currentTime := time.Now()
	formattedTime := currentTime.Format("2006-01-02 15:04:05")
	userToken := UserToken{}

	if err := conn.Where("user_id = ? AND expiration_time > ?", userId, formattedTime).First(&userToken).Error; err != nil {
